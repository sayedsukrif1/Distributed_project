...
func main() {
	...
	// New endpoint to get slave status
	http.HandleFunc("/slaves-status", func(w http.ResponseWriter, r *http.Request) {
		allowCORS(w)
		getSlavesStatus(w, r)
	})
...
// New function to check slaves status
func getSlavesStatus(w http.ResponseWriter, r *http.Request) {
	...
	for _, addr := range slaveAddresses {
		status := slaveStatus{Address: addr}
		client := &http.Client{Timeout: 2 * time.Second}
		_, err := client.Get(addr + "/ping")
		if err == nil {
			status.Status = "Alive"
			status.IsAlive = true
		} else {
...
// New function to monitor slaves periodically
func monitorSlaves() {
	...
	for range ticker.C {
		for _, addr := range slaveAddresses {
			go func(address string) {
				client := &http.Client{Timeout: 2 * time.Second}
				_, err := client.Get(address + "/ping")
				if err != nil {
					log.Printf("Slave %s is not responding", address)
				}
			}(addr)
		}
	}
...
// Modified replicateToSlaves to skip read-only slaves
func replicateToSlaves(path string) {
	for _, addr := range slaveAddresses {
		// Skip replication to slave1 (8002) for database operations
		if strings.HasSuffix(addr, "8002") && (strings.Contains(path, "/replicate/db") || strings.Contains(path, "/replicate/dropdb")) {
			continue
		}
...
// Modified replicateToSlavesJSON to skip read-only slaves for certain operations
func replicateToSlavesJSON(path string, data interface{}) {
	...
	for _, addr := range slaveAddresses {
		// Skip replication to slave1 (8002) for database operations
		if strings.HasSuffix(addr, "8002") && (path == "/replicate/db" || path == "/replicate/dropdb") {
			continue
		}
